...
const Cart = () => {
  ...
  const handleQuantityChange = async (id, size, variantId, newQuantity) => {
    if (newQuantity < 1) return;

    setUpdatingItems((prev) => ({
      ...prev,
      [`${id}-${size}-${variantId}`]: true,
    }));

    const result = await updateQuantity(id, size, newQuantity, variantId);

    setUpdatingItems((prev) => ({
      ...prev,
      [`${id}-${size}-${variantId}`]: false,
    }));
  ...
  const handleRemoveItem = async (id, size, variantId) => {
    setUpdatingItems((prev) => ({
      ...prev,
      [`${id}-${size}-${variantId}`]: true,
    }));

    const result = await removeFromCart(id, size, variantId);
              ...
              <div className="divide-y divide-gray-700">
                <AnimatePresence>
                  {items.map((item, index) => (
                    <motion.div
                      key={`${item.id}-${item.size}-${item.variantId || "default"}`}
                      initial={{ opacity: 0, y: 20 }}
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -20 }}
                          ...
                          <div className="flex items-center border border-gray-600 rounded-lg bg-gray-800/50">
                            <button
                              ...
                              disabled={
                                item.quantity <= 1 ||
                                updatingItems[
                                  `${item.id}-${item.size}-${item.variantId}`
                                ]
                              }
                            >
                              -
            ...
            <GlassCard className="p-6 backdrop-blur-xl sticky top-28">
              ...
              <div className="mt-8 space-y-4">
                ...
                <Link to="/products">
                  <AnimatedButton
                    variant="outline"
                    className="w-full py-3 border-gray-600 text-gray-300 hover:border-cyan-400 hover:text-cyan-400"
                  >
                    Continue Shopping
                  </AnimatedButton>
                </Link>
